...
class Trainer:
    ...
    def run_train_step(self):
        for data in self.data_loader:
            obs = data[0].to(self.cfg['device'])
            pred = data[1].to(self.cfg['device'])
            with torch.no_grad():
                ...
                obs_pad = padding_traj(obs, self.cfg['padding'], self.cfg['idx_pad'], self.cfg['zero_index'])
                obs_dct = torch.matmul(self.cfg['dct_m_all'][:self.cfg['n_pre']], obs)
                obs_dct_mod = torch.matmul(self.cfg['dct_m_all'][:self.cfg['n_pre']], obs_pad)
                if np.random.random() > self.cfg['mod_train']:
                    obs_dct_mod = None

            # train
            t = self.diffusion.sample_timesteps(obs.shape[0]).to(self.cfg['device'])
            x_t, noise = self.diffusion.noise_motion(obs_dct, t)
            predicted_noise = self.model(x_t, t, mod=obs_dct_mod)
            ...
            del loss, obs, obs_dct, obs_dct_mod, obs_pad, data
    ...
    def after_train_step(self):
        ...
        #pose_gen = pose_generator(self.data_loader, self.model, self.diffusion, self.cfg, mode='gif')
        #render_animation(self.skeleton, pose_gen, ['HumanMAC'], self.cfg['t_his'], ncol=4,
    ...
    def after_val_step(self):
        ...
        if self.iter % self.cfg['save_metrics_interval'] == 0 and self.iter != 0:
            if self.cfg['ema'] is True:
                compute_stats(self.diffusion, self.data_loader, self.ema_model, self.logger, self.cfg)
            else:
                compute_stats(self.diffusion, self.data_loader, self.model, self.logger, self.cfg)
